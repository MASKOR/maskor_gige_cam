    ...
    : it_(nh_)
{
    ...
    RawImagePub_ = it_.advertise("flir_camera/image_raw",1);
    ThermalImagePub_ = it_.advertise("flir_camera/thermal_image_raw",1);
...
void FlirNode::_showImages(){
    ...
    cv::imshow("image_raw", *RawImagePtr_);
    cv::imshow("thermal_image_raw", *ThermalImagePtr_);
...
{
    return (float)(pixelValue / 10.0f) - 273.15f;
}
